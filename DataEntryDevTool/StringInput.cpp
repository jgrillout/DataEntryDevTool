...
#pragma once
#include "DataEntry.h"
bool DataEntry::stringInput(DataEntry& dataEntry, std::ofstream& debugFile,int test) {
...
// This is the core logic that handles the editing of the string input field 
        ...
        case 331: //INSERT
            ...
            if (INSERT == true) {
                wattron(savewinFullScreen, COLOR_PAIR(2));
                mvwprintw(savewinFullScreen, rows - 2, cols - 8, "Insert");
            }
            else {
                wattron(savewinFullScreen, COLOR_PAIR(3));
                mvwprintw(savewinFullScreen, rows - 2, cols - 8, "       ");
                wattron(savewinFullScreen, COLOR_PAIR(2));
            }
            break;
...
// This is the  beginning of the core logic that handles building the string input field. 
            ...
            if (std::isprint(c)) {
                ...
                if (test == 24) {// input type is CHOICE
                    ...
                    if (dataEntry.allowedChoices.find(character) == std::string::npos) {
                        beep();
                        break;
                    }
                }
